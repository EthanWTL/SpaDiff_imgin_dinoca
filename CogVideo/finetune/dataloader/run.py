...
train_dl = DataLoader(train_ds, batch_size=1, shuffle=False, collate_fn=norm_collate)
val_dl   = DataLoader(val_ds,   batch_size=1, shuffle=False, collate_fn=norm_collate)
test_dl  = DataLoader(test_ds,  batch_size=1, shuffle=False, collate_fn=norm_collate)

# Check
batch = next(iter(train_dl))
print("Flow max after norm+clip:", batch["flow"].abs().max().item())  # <= 1.0
print("Flow shape:", batch["flow"].shape)  # Should be [B, T, 3, H, W]
